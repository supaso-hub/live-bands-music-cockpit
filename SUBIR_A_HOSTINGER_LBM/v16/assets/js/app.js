      ...
      <section class="glass-panel rounded-2xl p-6 shadow-xl space-y-4 border-l-4 border-l-amber-500">
        ...
        <div class="space-y-2 pt-2">
          ...
          <div id="help-ceremony-band" class="field-help-hint hidden">
            <span>Grupos exclusivos de Live Bands Music con repertorio especializado (clásico/cuerdas Vienna Brava, góspel contemporáneo Gospel On o baladas acústicas Latin Birds).</span>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ${renderCuratedBandChips("ceremony", ceremony.bandId, ["vienna-brava", "gospel-on", "latin-birds"])}
          </div>
        </div>
...
function renderCuratedBandChips(momentKey, selectedBandId, allowedIds) {
  ...
  return bands.map(b => {
    ...
    return `
      <div onclick="window.app.selectCuratedBand('${momentKey}', '${b.id}')" class="selectable-card glass-card rounded-xl p-3 border ${isSelected ? 'active' : 'border-gray-800'} cursor-pointer">
        ...
        <p class="text-[10px] text-gray-400 mt-1 line-clamp-2">${b.description}</p>
      </div>
    `;
  }).join("");
}
