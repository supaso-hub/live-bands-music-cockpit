...
import { fetchLead, hydrateCockpitFromLead } from "./services/lead-bridge.js";
...
function initApp() {
  ...
  // Detección e hidratación del Puente de Datos (Escaparate ➔ Cockpit)
  checkAndHydrateLeadBridge();
...
// ==========================================================================
// Integración del Puente de Datos con Token Corto (Escaparate ➔ Cockpit)
// ==========================================================================
async function checkAndHydrateLeadBridge() {
  if (typeof window === "undefined") return;
  const params = new URLSearchParams(window.location.search);
  const ref = params.get("ref");
  const inlineData = params.get("d") || params.get("lead");

  if (!ref && !inlineData) return;

  try {
    const lead = await fetchLead(ref, inlineData);
    if (lead) {
      const success = hydrateCockpitFromLead(lead, store);
      if (success) {
        const state = store.getState();
        renderFormStep1(state);
        renderMomentsConfig(state);
        renderTechOptions(state);
        renderPricingSummary(state);
        checkAcousticRules(state);
        checkTravelPolicy(state);
        updatePrintSheet(state);
        
        // Si viene con momentos seleccionados, avanzar directamente al paso de configuración musical
        setStep(1);

        // Mostrar notificación visual de precarga
        displayLeadHydrationBanner(lead);
      }
    }
  } catch (err) {
    console.warn("Fallo al hidratar lead bridge:", err);
  }
}

function displayLeadHydrationBanner(lead) {
  const existing = document.getElementById("lead-bridge-banner");
  if (existing) existing.remove();

  const mainContainer = document.querySelector("main") || document.querySelector(".max-w-7xl") || document.body;
  ...
  banner.innerHTML = `
    <div class="flex items-start gap-3.5">
      <div class="p-2.5 rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 shrink-0 mt-0.5">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/></svg>
      </div>
      <div>
        <div class="flex items-center gap-2 mb-1">
          <span class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
            ⚡ Anteproyecto Precargado del Escaparate • Ref: <strong class="text-white">${refCode}</strong>
          </span>
        </div>
        <h3 class="text-sm font-extrabold text-white">
          ${coupleName} — Presupuesto iniciado desde el Escaparate
        </h3>
        <p class="text-xs text-gray-300 mt-0.5">
          <strong>Finca:</strong> ${venue} &nbsp;•&nbsp; <strong>Fecha:</strong> ${date} &nbsp;•&nbsp; <em>Momento(s) preseleccionados cargados automáticamente.</em>
        </p>
      </div>
    </div>
    <button type="button" onclick="document.getElementById('lead-bridge-banner')?.remove()" class="text-gray-400 hover:text-white text-xs px-2 py-1 rounded bg-gray-800/60 hover:bg-gray-700 transition" title="Cerrar notificación">
      ✕
    </button>
  `;
